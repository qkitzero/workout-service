...
func TestCreateSet(t *testing.T) {
	...
	tests := []struct {
		...
		{"success create set", true, context.Background(), validExerciseID, 10, 60.0, time.Date(2024, 1, 1, 0, 0, 0, 0, time.UTC), nil},
		{"failure create set error", false, context.Background(), validExerciseID, 10, 60.0, time.Date(2024, 1, 1, 0, 0, 0, 0, time.UTC), fmt.Errorf("create set error")},
	}
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			mockSetUsecase.EXPECT().CreateSet(tt.ctx, tt.exerciseID, tt.rep, tt.weight, tt.trainedAt).Return(mockSet, tt.createSetErr).AnyTimes()
...
func TestListSets(t *testing.T) {
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			mockSetID := set.NewSetID()
			mockSet.EXPECT().ID().Return(mockSetID).AnyTimes()
			mockSet.EXPECT().ExerciseID().Return(exercise.NewExerciseID()).AnyTimes()
